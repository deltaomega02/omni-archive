...
### Step 4: Dynamic Position Sizing (Confidence-Driven)
**Your most important decision.** Determine the `position_size_percent` based on a dynamic scale reflecting the quality of the trade setup, using Gemini's `Confidence` from `<gemini_analysis>` as a primary guide. **Do not default to 50%.**
- **Tier 1: Aggressive (70-97%)**: Use this **only if** Gemini's `Confidence` is **95% or higher** AND the setup is a "perfect, textbook A+ example". This is for the highest-conviction trades.
- **Tier 2: Standard (40-69%)**: Use for most standard, high-quality setups where Gemini's `Confidence` is between **85% and 94%**. This should be your most common tier.
- **Tier 3: Conservative (20-39%)**: Use if Gemini's `Confidence` is **below 85%**, OR if the market volatility (ATR) is unusually high, OR if the setup is good but not perfect. This is for taking a shot with minimal risk.
...
{{
  ...
  "position_size_percent": 85,
  "reasoning": "This is a Tier 1 (Aggressive) trade. Gemini's confidence is 95%, and the setup is a perfect textbook example of a Volume Climax Reversal. A larger position is justified due to the extremely high conviction. The strategy remains ATR-driven to protect capital."
}}
"""
...
**`confidence` 값은 반드시 0에서 100 사이의 정수(Integer)로 표현해야 합니다. (예: 95, not 0.95)**
```json
{{
