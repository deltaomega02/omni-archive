...
class GPTClient:
    ...
    def create_strategy_phase3(self, 
            ...
            prompt = f"""
# AI Execution Strategist: Generate 2-Target Plan

# Goal: Calculate realistic entry, 2 targets (FIXED 70/30 split), initial stop, position size based on inputs. Output ONLY required JSON.
...
- **STRONG_BUY Pressure:**
    ...
    - MAJOR: **NO CHASE.** `Entry = Current Price * (1 - 0.003 to 0.005)` (Wait pullback)
- **BUY Pressure:**
...
**Target 1 (T1) - 70% Exit:**
- Find nearest significant Ask Wall within +3% of Entry.
- `IF` wall: `T1 = Wall_Price * 0.998`
- `ELSE`: `T1 = Entry * (1 + min(0.02, 1.0 * ATR / Entry))` (Realistic safe target)

**Target 2 (T2) - 30% Runner (Regime-Adjusted Realism):**
- Determine **Regime-Based ATR Multiplier**:
    - `BULL_TREND` or `KOREAN_PUMP`: Multiplier = 2.0
    - `SIDEWAYS`: Multiplier = 1.5
    - `BEAR_TRAP`: Multiplier = 1.0 (Very conservative)
- `Potential_T2 = Entry * (1 + Multiplier * ATR / Entry)`
- Find nearest significant `Next_Resistance` (Recent High or next Ask Wall).
- `T2 = min(Potential_T2, Next_Resistance * 0.998)`
- Ensure gap: `T2 = max(T2, T1 * 1.02)`

**Initial SL:**
- `SL = Entry * (1 - max(0.008, 1.0 * ATR / Entry))`
...
- Validate: T2 > T1 > Entry > SL. Check R:R (blended).
- **Output ONLY the JSON.**

# REQUIRED JSON OUTPUT
{{
  "entry_price": "[Calc Price Step 3]",
  "target_price_1": "[Calc T1 Step 4]",
  "target_price_2": "[Calc T2 Step 4]",
  "target_split_ratio": 0.7,
  "stop_loss_price": "[Calc SL Step 4]",
  "position_size_percent": "[Final % Step 5]",
  "reasoning": "[Profile: MAJOR/ALTCOIN]. Regime:{market_regime}. Pressure:{pressure}. Conf:{confidence}%. Strategy:[Entry logic]. T1(70%):[T1 logic]. T2(30%):[T2 logic, **mentioning regime multiplier used**, e.g., 'Runner aiming for +4.5% (1.5x ATR for SIDEWAYS regime) near Recent High']. Note:After T1, SL->Breakeven."
}}
"""
