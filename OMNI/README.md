...
### 실시간 대시보드
**시스템이 현재 실행 중이며, 실시간 거래 현황을 확인할 수 있습니다.**

**[OMNI 실시간 대시보드 바로가기](http://YOUR_SERVER_IP:8501/)**

대시보드에서 확인 가능한 정보:
- **실시간 거래 상태** - 현재 포지션, 수익률, 진행 중인 전략
- **거래 내역** - 최근 거래 이력, 매매 타이밍, 손익 차트
- **누적 수익** - 전체 거래 통계, 승률, 평균 수익률
- **AI 분석 현황** - Gemini/GPT-5가 현재 분석 중인 코인과 전략
- **시스템 상태** - 잔고, 마지막 업데이트 시간

### 운영 비용 및 성과 관리
**시스템 운영에 필요한 모든 비용과 실제 거래 성과를 투명하게 공개합니다.**

**[OMNI Project Hub (구글 시트)](<REMOVED-PRIVATE-SHEET>)**

관리 중인 항목:
- **월별 운영비** - GCP VM, API 사용료, 디스크 비용 등 실제 지출
- **재무 대시보드** - 누적 손익, 일평균 수익
- **일일 거래 로그** - 각 거래의 상세 내역 (진입가, 청산가, 수익률)
- **시스템 인프라** - 서버 사양, 네트워크 비용
...
### 2단계 목표가 시스템 (T1/T2 Strategy)

**목표**: 리스크를 점진적으로 감소시키면서 수익을 극대화

**동작 원리**:

**STAGE 1: 진입 대기**
- 계산된 진입가 도달 시 전량 매수 (35-90% 포지션)
- 최대 1시간 대기

**STAGE 2: T1 목표 추격** (전체 물량 보유)
- **목표**: T1 (Target 1) 도달
- **물량**: 100% 보유
- **손절가**: 원래 설정된 손절가 적용
- **T1 도달 시**: 
...
**STAGE 3: T2 목표 추격** (30% 물량 보유)
- **목표**: T2 (Target 2) 도달 - 더 높은 수익 추구
- **물량**: 30% 보유
- **손절가 이동**: **진입가 = 본절 (Breakeven)**
  - T1에서 이미 수익 확정했으므로 **리스크 ZERO**
  - 최악의 경우에도 손실 없음
- **T2 도달 시**:
  - 남은 30% 전량 청산 → **추가 수익 확정** (잭팟!)
- **본절 도달 시**:
  - 남은 30% 무손실 청산 → T1 수익은 그대로 유지

**예시 시나리오**:

| 진입가 | T1 목표가 | T2 목표가 | 원래 손절가 | 본절 손절가 |
|--------|-----------|-----------|-------------|-------------|
| 100,000원 | 103,500원 (+3.5%) | 107,000원 (+7.0%) | 97,500원 (-2.5%) | 100,000원 (±0%) |

**시나리오 1: T1만 도달**
- T1 도달 → 70% 청산 (+3.5%)
- 가격 하락 → 본절(100,000원) 도달 → 30% 무손실 청산
- **최종 수익**: +2.45% (70% × 3.5%)

**시나리오 2: T1 + T2 모두 도달 (잭팟!)**
- T1 도달 → 70% 청산 (+3.5%)
- T2 도달 → 30% 청산 (+7.0%)
- **최종 수익**: +4.55% (70% × 3.5% + 30% × 7.0%)

**시나리오 3: T1 미도달 손절**
- 가격 하락 → 원래 손절가(97,500원) 도달
- 100% 전량 청산
- **최종 손실**: -2.5%
...
**목표**: 3단계(STAGE) 시스템으로 계산된 전략을 실행하고 완료까지 모니터링

**STAGE 1: 진입 대기 및 매수**
1. **지정가 진입 시도**
   - 계산된 진입가 ±0.2% 도달 시 시장가 매수
   - 최대 1시간 대기
   - 타임아웃 시 거래 취소

**STAGE 2: T1 목표 추격** (전량 보유)
2. **실시간 모니터링** (5초 간격)
   - **T1 도달 시**:
     - 70% 물량 즉시 시장가 청산
     - 손절가를 진입가(본절)로 이동
