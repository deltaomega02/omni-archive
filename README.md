...
> **중첩된 `omni/` 폴더에 대해** — `OMNI/omni/`, `OMNI/omni/omni/`, `OMNI-fractal/omni/`
> 가 비슷해 보이지만 **같은 파일이 아니다.** 같은 모듈의 서로 다른 시점 버전이라
> (`ai/gpt_client.py` 부터 내용이 다르다) 정리하면 어느 쪽이 무엇이었는지가 사라진다.
> 아카이브라 그대로 둔다. 읽으실 때는 **`OMNI/omni/` 를 기준**으로 보시면 된다.

> **하위 폴더의 README 2개는 당시 작성된 운영 문서다.** "현재 실행 중" 같은 현재형 표현이
> 남아 있으나 시스템은 종료됐다. 최신 상태는 이 파일 기준이다.
