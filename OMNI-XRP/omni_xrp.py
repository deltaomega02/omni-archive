...
1. **교훈 우선 반영**: 위의 "축적된 핵심 교훈"을 이번 거래에 반드시 적용
2. **정확한 진입가 결정**: {signal_confidence['grade']}등급 신호에 최적화된 진입가
3. **XRP 전문가 활용**: 에너지 응축 및 위꼬리 패턴을 고려한 전략
4. **위꼬리 방어 설정**: XRP 특성상 위꼬리 방어 활성화 여부 결정
5. **동적 목표가/손절가**: 확정된 투자금과 XRP 변동성을 고려한 현실적 설정

**특별 지시**: 
- 확률론적 검증을 통과했으므로 entry_price는 반드시 0보다 큰 값으로 설정
- 🎓 **중요**: 축적된 교훈에서 진입/관리/타이밍 관련 조언을 반드시 이번 전략에 반영
...
목표: 확률론적 검증 결과 + 과거 교훈을 바탕으로 한 최적 실행 전략 수립"""
    ...
    def _request_ai_management_analysis_v72(self, market_data, market_regime, trigger_result):
        """v7.2 트리거 감지 시 GPT 기반 관리 분석 (교훈 반영)"""
        try:
            # NumPy 타입 변환
            market_data_serializable = convert_numpy_types(market_data)
            ...
            # 🎓 교훈 기반 컨텍스트
            learned_lessons = self._extract_learned_lessons()
            past_trades = self._get_recent_trades_with_lessons(limit=5)
            ...
            # 🤖 GPT 기반 포지션 관리 프롬프트
            management_prompt = f"""
    # OMNI-XRP v7.2 포지션 관리 AI 분석

    ## 🚨 트리거 감지 상황
    - **트리거 종류**: {trigger_result['trigger']}
    - **트리거 근거**: {trigger_result.get('evidence', '')}
    - **트리거 심각도**: {trigger_result.get('severity', 'MEDIUM')}

    ## 📊 현재 포지션 상태
    - **현재가**: {current_price:,.0f}원
    - **수익 상태**: {profit_status}
    - **시장 체제**: {market_regime['regime']}
    - **체제 신뢰도**: {market_regime['confidence']}

    ## 📊 실시간 시장 데이터
    {json.dumps(market_data_serializable, indent=2, ensure_ascii=False)}

    ## 🎓 축적된 핵심 교훈 (포지션 관리 필수 적용)
    {learned_lessons}
    ...
    위의 **{trigger_result['trigger']} 트리거 상황**에서 최적의 포지션 관리 전략을 수립해주세요:

    1. **교훈 우선 반영**: 축적된 교훈에서 포지션 관리 관련 조언을 이번 관리에 반드시 적용
    2. **트리거 대응**: {trigger_result['trigger']} 트리거의 심각도와 근거를 고려한 관리
    3. **수익 상태 고려**: 현재 {profit_status} 상황에 최적화된 관리
    4. **위꼬리 방어**: XRP 특성상 위꼬리 방어 시스템 지속 여부
    5. **동적 조정**: 목표가/손절가의 현실적 조정

    **특별 지시**:
    - entry_price = 0 (보유 중이므로)
    - 🎓 **중요**: 축적된 교훈에서 포지션 관리 실패/성공 사례를 반드시 반영
    - 트리거 종류별 최적 대응 전략 적용
    - 감정적 판단이 아닌 데이터 기반 냉정한 관리

    현재 {trigger_result['trigger']} 상황에 최적화된 v7.2 포지션 관리 전략을 JSON 형식으로 제시하세요.
    """
            
            # 🤖 GPT 호출 (포지션 관리 전용 스키마)
            response = self.client.chat.completions.create(
                model="gpt-4.1",
                messages=[
                    {
                        "role": "system",
                        "content": f"""당신은 OMNI-XRP v7.2의 포지션 관리 전문 AI입니다.

    핵심 역할:
    1. {trigger_result['trigger']} 트리거 상황에 최적화된 포지션 관리
    2. 과거 교훈을 현재 관리 결정에 반영
    3. 수익/손실 상태를 고려한 현실적 관리
    4. XRP 특성(위꼬리 방어)을 고려한 전략

    현재 상황: {market_regime['regime']} 체제에서 {trigger_result['trigger']} 트리거 감지
    목표: 교훈 기반 + 트리거 대응 최적 포지션 관리 전략 수립"""
                    },
                    {"role": "user", "content": management_prompt}
                ],
                response_format={
                    "type": "json_schema", 
                    "json_schema": self._get_management_response_schema_v72()
                },
                max_tokens=1800,
                temperature=0.1
            )
            ...
            # v7.2 추가 정보 포함
            management_advice['change_trigger'] = trigger_result['trigger']
            management_advice['trigger_evidence'] = trigger_result.get('evidence', '')
            management_advice['wick_defense_status'] = "지속 활성화"
            
            logger.info(f"✅ v7.2 GPT 기반 + 교훈 반영 관리 조언 생성 완료: {trigger_result['trigger']}")
            return management_advice
            
        except Exception as e:
            logger.error(f"❌ v7.2 GPT 기반 관리 조언 생성 중 오류: {e}")
            return None
    ...
    def reflect_single_trade(self, completed_trade_id):
        ...
        try:
            ...
            if reflection_analysis:
                ...
                # 🆕 5단계: 교훈을 다음 거래 적용 시스템에 저장
                self._apply_lessons_to_next_trade(completed_trade_id, reflection_analysis)
                
                logger.info(f"✅ v7.2 거래 ID {completed_trade_id} 실전 교훈 추출 및 적용 시스템 저장 완료")
                return reflection_analysis
            else:
                logger.error(f"❌ v7.2 거래 ID {completed_trade_id} 교훈 추출 실패")
                return None
                
        except Exception as e:
            logger.error(f"❌ v7.2 교훈 기반 회고 중 오류: {e}")
            return None
    ...
    def _perform_comprehensive_reflection_v72(self, completed_trade, position_history):
        ...
        try:
            ...
            # 보유 기간
            holding_duration = self._calculate_holding_duration(entry_time, exit_time)
            
            # 실제 수익률
            real_profit_rate = ((actual_exit - real_entry_price) / real_entry_price * 100)
            
            # 📊 실전 교훈 중심 GPT 프롬프트 생성
            practical_prompt = f"""
    # OMNI-XRP v7.2 실전 교훈 추출 (거래 ID: {t_id})

    당신은 실전 트레이딩 교훈 전문가입니다. 다음 거래에서 **즉시 적용 가능한 구체적 교훈**을 추출해주세요.

    ## 📊 거래 기본 정보
    - **실제 진입가**: {real_entry_price:,.0f}원 → **실제 매도가**: {actual_exit:,.0f}원  
    - **최종 수익률**: {real_profit_rate:+.2f}% ({profit:+,.0f}원)
    - **보유 기간**: {holding_duration}
    - **거래 결과**: {result}
    - **관리 변경**: {len(management_changes)}회

    ## 🎯 Tree of Thoughts 방식 10단계 실전 분석

    다음 **10가지 핵심 관점**에서 전체 포지션 히스토리를 심층 분석해주세요:

    ### 1️⃣ 실제 진입 전략 평가 
    **진입가**: {real_entry_price:,.0f}원, **이유**: {real_entry_reason[:50]}...
    - ✅ **무엇이 좋았나**: 진입 전략의 성공 요소
    - ❌ **무엇이 문제였나**: 진입 전략의 실패 요소  
    - 🔧 **다음엔 어떻게**: 진입 전략 개선 방향

    ### 2️⃣ 포지션 관리 적절성
    **목표가/손절가 변경**: {len(management_changes)}회
    - ✅ **잘한 관리**: 수익에 도움된 관리 결정
    - ❌ **잘못된 관리**: 손실을 키운 관리 결정
    - 🔧 **개선 방향**: 관리 의사결정 프로세스 개선

    ### 3️⃣ 관리 계획 변경의 타당성
    각 변경이 수익에 미친 영향 분석
    - ✅ **효과적 변경**: 실제로 도움된 변경들
    - ❌ **무의미 변경**: 결과에 도움 안된 변경들  
    - 🔧 **변경 기준**: 언제 관리를 변경해야 하는가

    ### 4️⃣ 보유 기간 최적성
    **보유 기간**: {holding_duration}
    - ✅ **적절한 타이밍**: 보유 기간이 적절했던 이유
    - ❌ **부적절한 타이밍**: 더 일찍/늦게 팔았어야 하는 이유
    - 🔧 **타이밍 개선**: 보유 기간 최적화 방법

    ### 5️⃣ 실제 수익률 달성도  
    **수익률**: {real_profit_rate:+.2f}%
    - ✅ **목표 달성**: 예상 수익률과 비교한 성과
    - ❌ **목표 미달**: 예상보다 낮은/높은 이유
    - 🔧 **기대치 조정**: 현실적 수익률 목표 설정

    ### 6️⃣ 리스크 관리 효과성
    손절가 조정 히스토리
    - ✅ **효과적 방어**: 리스크를 잘 관리한 부분
    - ❌ **방어 실패**: 리스크 관리 미흡한 부분
    - 🔧 **방어 강화**: 리스크 관리 개선 방향

    ### 7️⃣ 관리 계획 변경에서 얻은 교훈
    - ✅ **성공 패턴**: 반복하면 좋을 관리 패턴
    - ❌ **실패 패턴**: 피해야 할 관리 패턴  
    - 🔧 **패턴 적용**: 다음 거래에 즉시 적용할 패턴

    ### 8️⃣ 다음 거래 시 관리 개선 방향
    - ✅ **강화할 점**: 계속 유지하고 강화할 관리 방식
    - ❌ **수정할 점**: 반드시 바꿔야 할 관리 방식
    - 🔧 **새로운 시도**: 시도해볼 새로운 관리 방법

    ### 9️⃣ AI 조언 vs 실제 결과 비교  
    - ✅ **AI가 맞춘 것**: AI 조언이 정확했던 부분
    - ❌ **AI가 틀린 것**: AI 조언이 부정확했던 부분
    - 🔧 **AI 개선**: AI 성능 향상을 위한 피드백

    ### 🔟 전체 포지션 운영 종합 평가
    - ✅ **이번 거래 성공 요인**: 가장 중요한 성공 요인
    - ❌ **이번 거래 실패 요인**: 가장 치명적인 실패 요인  
    - 🔧 **종합 개선 방향**: 전체적인 거래 실력 향상 방향

    ---

    ## 🎓 최종 실전 교훈 (5가지)

    위 10단계 분석을 바탕으로 **다음 거래에서 즉시 적용할 구체적 교훈 5가지**를 제시:
    ...
    **각 교훈은 구체적이고 실행 가능해야 하며, 다음 거래에서 바로 체크할 수 있는 명확한 액션이어야 합니다.**
    """
            ...
            reflection_response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system", 
                        "content": """당신은 실전 트레이딩 교훈 추출 전문가입니다.

    핵심 역할:
    1. 복잡한 기술분석이 아닌 **실제 거래에서 바로 쓸 수 있는 교훈** 추출
    2. 다음 거래에서 **즉시 체크하고 적용할 수 있는 구체적 액션** 제시  
    3. 감정적, 심리적 요소까지 포함한 **인간적인 거래 개선점** 발견
    4. 이론이 아닌 **실전에서 검증 가능한 개선 방향** 제시

    분석 원칙:
    - 무엇을 잘했고 무엇을 못했는지 명확히 구분
    - 각 실패에 대한 구체적이고 실행 가능한 해결책 제시
    - 다음 거래에서 바로 적용할 수 있는 체크리스트 형태로 정리
    - 복잡한 지표 분석보다는 단순하고 명확한 규칙 중심

    목표: 다음 거래에서 더 나은 결과를 위한 실행 가능한 개선점 5가지 도출"""
                    },
            ...
            logger.error(f"❌ v7.2 실전 교훈 추출 중 오류: {e}")
            return None

    def _apply_lessons_to_next_trade(self, trade_id, reflection_content):
        """회고에서 추출한 교훈을 다음 거래에 즉시 적용하는 시스템"""
        try:
            logger.info(f"🎓 거래 ID {trade_id} 교훈을 다음 거래 적용 시스템에 저장 중...")
            
            # 교훈 파일 저장 경로
            lessons_file = "active_lessons.json"
            ...
            new_lesson = {
                ...
                'lesson_timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'reflection_summary': reflection_content[:500] + "...",  # 요약만 저장
                'status': 'ACTIVE',
                'applied_count': 0
            }
            
            active_lessons.insert(0, new_lesson)  # 최신 교훈을 앞에 추가
            active_lessons = active_lessons[:10]  # 최대 10개만 유지
            
            # 교훈 파일 업데이트
            with open(lessons_file, 'w', encoding='utf-8') as f:
                json.dump(active_lessons, f, ensure_ascii=False, indent=2)
            
            logger.info(f"✅ 거래 ID {trade_id} 교훈이 활성 교훈 시스템에 추가됨")
            
            return True
            
        except Exception as e:
            logger.error(f"❌ 교훈 적용 시스템 저장 중 오류: {e}")
            return False

    def _get_active_lessons_for_strategy(self):
        """전략 수립 시 활성 교훈들을 가져와서 적용"""
        try:
            lessons_file = "active_lessons.json"
            ...
            if not active_lessons:
                return "현재 활성화된 교훈이 없습니다."
            
            # 최근 5개 교훈을 요약해서 반환
            lesson_summary = "🎓 최근 활성 교훈들 (다음 거래에 활용할 것):\n\n"
            
            for i, lesson in enumerate(active_lessons[:5], 1):
                source_trade = lesson['source_trade_id']
                timestamp = lesson['lesson_timestamp']
                summary = lesson['reflection_summary']
                applied_count = lesson.get('applied_count', 0)
                
                lesson_summary += f"{i}. 거래 {source_trade}에서 학습 ({timestamp})\n"
                lesson_summary += f"   적용 횟수: {applied_count}회\n"
                lesson_summary += f"   핵심 교훈: {summary}\n\n"
            
            return lesson_summary
            
        except Exception as e:
            logger.error(f"❌ 활성 교훈 조회 중 오류: {e}")
            return "교훈 조회 실패"
...
"""
    ...
    def _extract_learned_lessons(self):
        """활성 교훈 시스템에서 교훈 추출 (기존 메서드 대체)"""
        try:
            # 새로운 활성 교훈 시스템 사용
            active_lessons = self._get_active_lessons_for_strategy()
            
            # 기본 v7.2 시스템 교훈과 결합
            base_lessons = """
    🎯 OMNI-XRP v7.2 기본 원칙 (Base Principles):

    1. **시장 선행지표 확인 (Market Leading Indicators First):**
       - BTC의 추세가 불분명하거나 주요 거시 경제/소송 뉴스 발표 직전에는 진입을 보수적으로 판단한다.
       - XRP 단독 호재보다는 시장 전체의 건전성을 최우선으로 고려한다.

    2. **변동성 기반 리스크 관리 (Volatility-Based Risk Management):**
       - 일일 변동성(ATR)이 5%를 초과하는 '극단적 변동성' 구간에서는 목표 수익률을 낮추고, 손절 라인을 타이트하게 설정한다.
       - 변동성이 낮은 '수축 구간'에서는 돌파 매매를 준비하되, 돌파 실패 시 즉시 이탈한다.

    3. **'위꼬리' 함정 방어 ('Wick' Trap Defense):**
       - 거래량이 실리지 않은 급등은 추격 매수를 금지한다. 이는 전형적인 유동성 흡수(Liquidity Grab) 함정일 확률이 높다.
       - 강한 저항선 근처에서는 상승 돌파를 확인하기 전까지 분할 매도로 대응하는 것을 원칙으로 한다.

    4. **신호의 교차 검증 (Cross-Validation of Signals):**
       - 단일 지표(예: RSI 과매수)만으로 판단하지 않는다. 최소 2개 이상의 다른 지표 그룹(예: 추세, 거래량)에서 확인된 신호만 유효 거래 기회로 간주한다.
       - 다중 시간대(Multi-Timeframe) 분석을 통해 단기 신호가 장기 추세에 순응하는지 반드시 확인한다.
    """
