...
"""
    ...
    def get_optimized_market_data(self) -> Optional[Dict]:
        """
        v8.0 API 최적화된 시장 데이터 수집 (각 시간대별 직접 호출)
        ...
        if self._is_cache_valid():
            self.logger.log_with_context('info', '캐시된 시장 데이터 사용')
        ...
        try:
            ...
            # 각 시간대별 데이터를 직접 호출
            timeframe_data = {}
            api_calls = 2  # orderbook + balances
            ...
            if df_day is not None and len(df_day) > 0:
                timeframe_data['day'] = df_day
                api_calls += 1
            
            # 최소한의 데이터 확인
            if len(timeframe_data) == 0:
                self.logger.log_with_context('error', 'OHLCV 데이터 조회 실패')
                return None
...
## =============================================================================
    ...
    def _analyze_xrp_expert_patterns_v8(self, timeframe_data: Dict[str, pd.DataFrame], current_price: float) -> Dict:
            """
            v8.0 XRP 전문가 패턴 분석 (직접 호출된 데이터 사용)
            
            Args:
                timeframe_data (Dict[str, pd.DataFrame]): 시간대별 데이터
            ...
            """
            try:
                self.logger.log_with_context('info', 'XRP 전문가 패턴 분석 시작')
    ...
    def _calculate_comprehensive_indicators_v8(self, timeframe_data: Dict[str, pd.DataFrame]) -> Dict:
        """
        v8.0 포괄적 기술적 지표 계산 (직접 호출된 데이터 사용)
        ...
        for tf in timeframes:
            ...
            try:
                ...
                # OHLC 데이터는 float64로 변환
                for col in ['open', 'high', 'low', 'close']:
                    if col in df.columns:
                        df[col] = pd.to_numeric(df[col], errors='coerce').astype('float64')
                
                # Volume 처리 - float64로 유지 (int 변환 시 오버플로우 방지)
                ...
                if len(df) < 20:  # 최소 데이터 확인
                    self.logger.log_with_context('warning', f'{tf} 유효한 데이터 부족 (처리 후 {len(df)}개)')
                    continue
                ...
                # 실패 시 기본값 제공
                indicators[tf] = self._get_fallback_indicators_v8(current_price if 'current_price' in locals() else 0)
    ...
    def _calculate_volume_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # volume을 float로 확실히 변환
            df_copy['volume'] = df_copy['volume'].astype('float64')
            
            # 거래량 이동평균
            volume_sma_20 = ta.sma(df_copy['volume'], length=20)
            
            # 거래량 비율
            current_volume = float(df_copy['volume'].iloc[-1])
            volume_ratio = 1.0
            if not pd.isna(volume_sma_20.iloc[-1]) and volume_sma_20.iloc[-1] > 0:
                volume_ratio = current_volume / float(volume_sma_20.iloc[-1])
            
            # OBV (On-Balance Volume)
            obv = ta.obv(df_copy['close'], df_copy['volume'])
            
            # VWAP (Volume Weighted Average Price)
            vwap = ta.vwap(df_copy['high'], df_copy['low'], df_copy['close'], df_copy['volume'])
            
            # MFI (Money Flow Index) - 타입 문제 해결
            try:
                # MFI 계산 전 데이터 타입 확인 및 변환
                high_float = df_copy['high'].astype('float64')
                low_float = df_copy['low'].astype('float64')
                close_float = df_copy['close'].astype('float64')
                volume_float = df_copy['volume'].astype('float64')
                
                mfi = ta.mfi(high_float, low_float, close_float, volume_float, length=14)
            except Exception as e:
                self.logger.log_with_context('warning', f'MFI 계산 실패: {e}')
                mfi = pd.Series([50.0] * len(df_copy))  # 기본값
            
            # A/D Line
            ad = ta.ad(df_copy['high'], df_copy['low'], df_copy['close'], df_copy['volume'])
            
            # Chaikin Money Flow
            cmf = ta.cmf(df_copy['high'], df_copy['low'], df_copy['close'], df_copy['volume'], length=20)
            ...
            return {
                ...
                'vwap': self._safe_float(vwap.iloc[-1], df_copy['close'].iloc[-1]),
                'mfi': self._safe_float(mfi.iloc[-1] if mfi is not None else 50, 50),
            ...
            return self._get_default_volume_indicators(df['close'].iloc[-1] if 'df' in locals() else 0)
...
## =============================================================================
    ...
    def _validate_entry_checklist_v8(self, market_data: Dict, market_regime: Dict) -> Dict:
        ...
        try:
            ...
            self.logger.log_with_context(
                'info',
                f'✅ 1단계 시장체제: {checklist["market_regime"]:.1f}/1.0점',
                regime=regime,
            ...
            self.logger.log_with_context(
                'info',
                f'✅ 2단계 추세일치: {checklist["trend_alignment"]:.1f}/1.0점',
                aligned=f'{aligned_timeframes}/{total_timeframes}',
                ratio=f'{alignment_ratio:.1%}' if total_timeframes > 0 else 'N/A'
            )
            ...
            self.logger.log_with_context(
                'info',
                f'✅ 3단계 신호강도: {checklist["signal_strength"]:.1f}/1.0점',
                signal_count=f'{signal_count}개'
            )
            ...
            self.logger.log_with_context(
                'info',
                f'✅ 4단계 반대신호: {checklist["no_contrary_signals"]:.1f}/1.0점',
                contrary_count=f'{contrary_signals}개'
            )
            ...
            self.logger.log_with_context(
                'info',
                f'✅ 5단계 손익비: {checklist["risk_reward"]:.1f}/1.0점',
            ...
            self.logger.log_with_context(
                'info',
                f'✅ 6단계 XRP보너스: {checklist["xrp_expert_bonus"]:.1f}/0.5점',
                expert_confidence=f'{expert_confidence}/5',
                energy_compression=energy_compression,
                breakout_prob=f'{breakout_prob:.0%}'
            )
            ...
            self.logger.log_with_context(
                'info',
                f'📊 체크리스트 총점: {total_score:.1f}/5.5점',
                result='✅ 통과' if total_score >= 2.0 else '❌ 탈락',
    ...
    def _calculate_signal_confidence_multiplier_v8(self, checklist_score: float) -> Dict:
        ...
        try:
            ...
            elif checklist_score >= 2.0:  
                multiplier = 0.2           
                grade = "C"               
                description = "약한 신호 - 매우 보수적 접근 (20% 투자비중)"
            else:
                multiplier = 0.0
                grade = "F"
    ...
    def orient_and_decide_v8(self, market_data: Dict) -> Optional[Dict]:
        ...
        """
        try:
            self.logger.log_with_context('info', 'orient_and_decide_v8 함수 진입')
            ...
            import traceback
            self.logger.log_with_context('error', f'상세 오류: {traceback.format_exc()}')
            return None
    ...
    def _generate_new_entry_strategy_v8(self, market_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.log_with_context(
                ...
                regime=market_regime['regime'],
                confidence=market_regime['confidence'],
                regime_score=f"{market_regime.get('regime_score', 0):.2f}/5"
            )
            ...
            self.logger.log_with_context(
                'info',
                f'📊 신호 신뢰도: {signal_confidence["grade"]}등급',
                multiplier=f'{signal_confidence["multiplier"]:.1f}x',
                description=signal_confidence['description']
            )
            
            # 4. v8.0 하드 임계값 검사 (2.0점 미만 진입 금지)
            if checklist_result['total_score'] < 2.0:
                self.logger.log_with_context(
                    'info',
                    f'🚫 진입 거부: 체크리스트 점수 {checklist_result["total_score"]:.1f} < 2.0 (최소 요구점수)'
                )
                self._log_detailed_rejection_analysis_v8(market_regime, checklist_result, signal_confidence, market_data)
    ...
    def _log_detailed_rejection_analysis_v8(self, market_regime: Dict, checklist_result: Dict, 
                                        signal_confidence: Dict, market_data: Dict) -> None:
        """
        체크리스트 점수 미달 시 상세 분석 로깅
        ...
        try:
            ...
            self.logger.log_with_context(
                'warning',
                f'🚫 v8.0 체크리스트 점수 미달로 진입 거부',
                total_score=f'{total_score:.1f}/5.5점',
                grade=signal_confidence['grade'],
                threshold='2.0점',
                gap=f'{2.0 - total_score:.1f}점 부족'
            )
            ...
            for category, score in checklist_items.items():
                max_score = 1.0 if category != 'xrp_expert_bonus' else 0.5
                score_pct = (score / max_score) * 100
                
                if score_pct < 25:  # 25% 미만
                    failing_categories.append(f"{category}: {score:.1f}/{max_score} ({score_pct:.0f}%)")
                elif score_pct < 50:  # 50% 미만
                    weak_categories.append(f"{category}: {score:.1f}/{max_score} ({score_pct:.0f}%)")
            
            if failing_categories:
                self.logger.log_with_context(
                    'warning',
                    '❌ 심각한 문제 항목들',
                    failing_items=failing_categories
                )
            ...
            if weak_categories:
                self.logger.log_with_context(
                    'info',
                    '⚠️ 약한 신호 항목들',
                    weak_items=weak_categories
                )
            
            # 3단계: 시장 상황 요약
            regime = market_regime.get('regime', '알수없음')
            regime_score = market_regime.get('regime_score', 0)
            btc_analysis = market_regime.get('btc_analysis', {})
            ...
            self.logger.log_with_context(
                ...
                regime_score=f'{regime_score:.2f}/5',
                btc_influence=btc_analysis.get('btc_influence', '알수없음'),
                btc_change=f"{btc_analysis.get('btc_1h_change', 0):+.1f}%"
            )
            ...
            self.logger.log_with_context(
                ...
                '🔍 XRP 전문가 시스템 상태',
                energy_compression=xrp_analysis.get('energy_compression_detected', False),
                breakout_probability=f"{xrp_analysis.get('breakout_probability', 0):.0%}",
                wick_risk=xrp_analysis.get('wick_pattern_risk', 'unknown'),
                expert_confidence=f"{xrp_analysis.get('expert_confidence', 0)}/5",
                dominant_pattern=xrp_analysis.get('dominant_pattern', 'NONE')
            )
            
            # 5단계: 개선 제안 로깅
            improvement_suggestions = self._generate_improvement_suggestions_v8(checklist_items, market_regime)
            ...
            # 6단계: 다음 기회 예측
            next_opportunity_analysis = self._predict_next_opportunity_v8(checklist_result, market_regime)
            
            self.logger.log_with_context(
                'info',
                '⏰ 다음 기회 예측',
                **next_opportunity_analysis
            )
            
        except Exception as e:
            self.logger.log_with_context('error', f'상세 거부 분석 로깅 중 오류: {e}')

    def _generate_improvement_suggestions_v8(self, checklist_items: Dict, market_regime: Dict) -> List[str]:
        """
        체크리스트 결과 기반 개선 제안 생성
        
        Args:
            checklist_items (Dict): 체크리스트 항목별 점수
            market_regime (Dict): 시장 체제 분석
        ...
        try:
            # 시장체제 개선
            if checklist_items.get('market_regime', 0) < 0.5:
                regime = market_regime.get('regime', '')
                if '하락장' in regime:
                    suggestions.append("시장체제가 하락장 → 상승 전환 신호 대기 필요")
                elif '혼조장' in regime:
                    suggestions.append("혼조장 상황 → 명확한 방향성 확인 후 진입")
                else:
                    suggestions.append("시장체제 신뢰도 향상 필요")
            
            # 추세 일치성 개선
            if checklist_items.get('trend_alignment', 0) < 0.5:
                suggestions.append("다중 시간대 추세 불일치 → 주요 시간대 추세 정렬 대기")
            
            # 신호 강도 개선
            if checklist_items.get('signal_strength', 0) < 0.5:
                suggestions.append("진입 신호 부족 → 골든크로스, 거래량 급증, RSI 반등 등 추가 신호 대기")
            
            # 반대 신호 해결
            if checklist_items.get('no_contrary_signals', 0) < 0.5:
                suggestions.append("반대 신호 감지됨 → BTC 급락, 베어리시 다이버전스 등 해소 대기")
            
            # 손익비 개선
            if checklist_items.get('risk_reward', 0) < 0.5:
                suggestions.append("손익비 부족 → 더 좋은 진입점 또는 목표점 대기")
            
            # XRP 전문가 보너스 개선
            if checklist_items.get('xrp_expert_bonus', 0) < 0.2:
                suggestions.append("XRP 특화 신호 부족 → 에너지 압축, 패턴 완성 대기")
            
            return suggestions
            
        except Exception as e:
            return [f"개선 제안 생성 중 오류: {e}"]

    def _predict_next_opportunity_v8(self, checklist_result: Dict, market_regime: Dict) -> Dict:
        """
        다음 기회 시점 예측
        ...
        try:
            total_score = checklist_result['total_score']
            needed_score = 2.0 - total_score
            
            prediction = {
                'needed_improvement': f'{needed_score:.1f}점',
                'estimated_timeframe': '알수없음',
                'key_waiting_factors': []
            }
            ...
            if regime_score < -1:
                prediction['estimated_timeframe'] = '2-4시간 이상 (강한 하락 추세)'
                prediction['key_waiting_factors'].append('하락 추세 반전')
            elif regime_score < 0:
                prediction['estimated_timeframe'] = '30분-2시간 (약한 하락)'
                prediction['key_waiting_factors'].append('횡보 전환')
            else:
                prediction['estimated_timeframe'] = '10-30분 (상승 기조)'
                prediction['key_waiting_factors'].append('신호 강화')
            
            # 점수 갭 기반 조정
            if needed_score > 1.0:
                prediction['estimated_timeframe'] = '1시간 이상 (대폭 개선 필요)'
            elif needed_score > 0.5:
                prediction['estimated_timeframe'] = '20-60분 (중간 개선 필요)'
            else:
                prediction['estimated_timeframe'] = '5-20분 (소폭 개선 필요)'
            
            # 주요 대기 요소
            checklist_items = checklist_result.get('checklist', {})
            
            if checklist_items.get('market_regime', 0) < 0.3:
                prediction['key_waiting_factors'].append('시장체제 개선')
            if checklist_items.get('signal_strength', 0) < 0.3:
                prediction['key_waiting_factors'].append('기술적 신호 출현')
            if checklist_items.get('no_contrary_signals', 0) < 0.5:
                prediction['key_waiting_factors'].append('반대 신호 해소')
            
            return prediction
            
        except Exception as e:
            return {
                'needed_improvement': '계산실패',
                'estimated_timeframe': '알수없음',
                'key_waiting_factors': [f'예측 오류: {e}']
            }
    ...
    def _create_no_entry_response_v8(self, market_regime: Dict, checklist_result: Dict, signal_confidence: Dict) -> Dict:
        ...
        return {
            ...
            "target_price": 0,
            "stop_loss_price": 0,
            "entry_reason": f"v8.0 확률론적 검증 실패 ({grade}등급, {total_score:.1f}/5.5점) - 하드 임계값 2.0점 미달",
    ...
    def _request_ai_entry_analysis_v8(self, market_data: Dict, market_regime: Dict, 
        ...
        try:
                    ...
                    "json_schema": {
                        ...
                        "schema": {
                            ...
                            "properties": {
                                "entry_price": {
                                    "type": "number",
                                    "description": "진입 가격 (원 단위)"
                                },
                                "target_price": {
                                    "type": "number",
                                    "description": "목표 가격 (원 단위)"
                                },
                                "stop_loss_price": {
                                    "type": "number",
                                    "description": "손절 가격 (원 단위)"
                                },
                                "entry_reason": {
                                    "type": "string",
                                    "description": "진입 근거 설명"
                                },
                                "target_reason": {
                                    "type": "string",
                                    "description": "목표가 설정 근거"
                                },
                                "stop_loss_reason": {
                                    "type": "string",
                                    "description": "손절가 설정 근거"
                                },
                                "sell_strategy": {
                                    "type": "string",
                                    "description": "구체적인 매도 전략"
                                },
                                "lessons_applied": {
                                    "type": "string",
                                    "description": "이번 거래에 적용한 교훈"
                                }
                            },
                            "required": [
                                "entry_price", "target_price", "stop_loss_price",
                                "entry_reason", "target_reason", "stop_loss_reason",
                                "sell_strategy", "lessons_applied"
                            ],
            ...
            try:
                ...
                parsed_strategy = json.loads(ai_response)
                
                # 디버그 로그
                self.logger.log_with_context(
                    'debug',
                    'AI JSON 응답 파싱 성공',
                    entry=parsed_strategy.get('entry_price'),
                    target=parsed_strategy.get('target_price'),
                    stop=parsed_strategy.get('stop_loss_price')
                )
                
            except json.JSONDecodeError as e:
                self.logger.log_with_context('error', f'AI JSON 응답 파싱 실패: {e}')
                self.logger.log_with_context('error', f'AI 원본 응답: {response.choices[0].message.content}')
                return None
            
            # 기존 _parse_ai_entry_response_v8 대신 직접 처리
            current_price = market_data['current_price']
            
            # 유효성 검증
            if not self._validate_prices_v8(
                current_price,
                parsed_strategy['entry_price'],
                parsed_strategy['target_price'],
                parsed_strategy['stop_loss_price']
            ):
                return None
            
            # v8.0 확률론적 정보 추가
            final_strategy = {
                'entry_price': parsed_strategy['entry_price'],
                'target_price': parsed_strategy['target_price'],
                'stop_loss_price': parsed_strategy['stop_loss_price'],
                'entry_reason': parsed_strategy['entry_reason'],
                'target_reason': parsed_strategy['target_reason'],
                'stop_loss_reason': parsed_strategy['stop_loss_reason'],
                'sell_strategy': parsed_strategy['sell_strategy'],
                'lessons_applied': parsed_strategy['lessons_applied'],
                ...
                'regime_adaptation': f"시장 체제 '{market_regime.get('regime', '분석중')}'에 최적화된 전략"
            }
            
            self.logger.log_with_context(
                'info',
                'v8.0 AI 정교 분석 완료',
                entry_price=f'{final_strategy["entry_price"]:,.0f}원',
                target_price=f'{final_strategy["target_price"]:,.0f}원',
                expected_return=f'{((final_strategy["target_price"]/final_strategy["entry_price"]-1)*100):+.1f}%'
            )
...
응답 형식:
반드시 JSON 형식으로 응답하세요. 모든 가격은 숫자로, 설명은 문자열로 제공합니다.
...
목표: v8.0 시스템이 **높은 성공률**과 **안정적인 수익**을 달성할 수 있는 정교한 전략 제공"""
    ...
    def _analyze_bitcoin_correlation_v8(self, market_data: Dict) -> Dict:
        """
        v8.0 비트코인 상관관계 분석 (실제 BTC 데이터 사용으로 정확도 극대화)
        ...
        try:
            btc_analysis = {
                ...
                'btc_volatility_risk': 'low'
            }
            
            # 🔥 실제 BTC 데이터 수집 (1시간봉 24개 = 24시간)
            btc_data = self._api_call_wrapper(
                pyupbit.get_ohlcv, 
                "KRW-BTC", 
                interval="minute60", 
                count=24
            ...
            if btc_data is None or len(btc_data) < 5:
                self.logger.log_with_context('warning', 'BTC 데이터 조회 실패 - XRP 변동성으로 추정')
                return self._fallback_btc_analysis_v8(market_data)
            
            # 1시간 변동률 계산 (정확한 BTC 변동률)
            current_btc_price = float(btc_data['close'].iloc[-1])
            prev_1h_btc_price = float(btc_data['close'].iloc[-2])
            btc_1h_change = ((current_btc_price - prev_1h_btc_price) / prev_1h_btc_price * 100)
            
            # 4시간 변동률 계산 (더 넓은 시각)
            if len(btc_data) >= 5:
                prev_4h_btc_price = float(btc_data['close'].iloc[-5])
                btc_4h_change = ((current_btc_price - prev_4h_btc_price) / prev_4h_btc_price * 100)
            else:
                btc_4h_change = btc_1h_change * 2  # 추정값
            
            # BTC 변동성 분석 (ATR 기반)
            btc_high = btc_data['high'].tail(10)
            btc_low = btc_data['low'].tail(10)
            btc_close = btc_data['close'].tail(10)
            
            # True Range 계산
            tr1 = btc_high - btc_low
            tr2 = abs(btc_high - btc_close.shift(1))
            tr3 = abs(btc_low - btc_close.shift(1))
            true_range = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)
            atr = true_range.mean()
            
            atr_ratio = (atr / current_btc_price * 100) if current_btc_price > 0 else 0
            
            # 🎯 정확한 BTC 영향도 결정
            if abs(btc_1h_change) >= 4.0:
                btc_analysis['btc_influence'] = '매우높음'
            elif abs(btc_1h_change) >= 2.5:
                btc_analysis['btc_influence'] = '높음'
            elif abs(btc_1h_change) >= 1.5:
                btc_analysis['btc_influence'] = '중간'
            else:
                btc_analysis['btc_influence'] = '낮음'
            
            # BTC 추세 방향 결정
            if btc_1h_change > 2:
                btc_analysis['btc_trend'] = 'strong_bullish'
            elif btc_1h_change > 0.5:
                btc_analysis['btc_trend'] = 'bullish'
            elif btc_1h_change < -2:
                btc_analysis['btc_trend'] = 'strong_bearish'
            elif btc_1h_change < -0.5:
                btc_analysis['btc_trend'] = 'bearish'
            else:
                btc_analysis['btc_trend'] = 'neutral'
            
            # 변동성 위험도 평가
            if atr_ratio > 2.5:
                btc_analysis['btc_volatility_risk'] = 'very_high'
            elif atr_ratio > 1.5:
                btc_analysis['btc_volatility_risk'] = 'high'
            elif atr_ratio > 1.0:
                btc_analysis['btc_volatility_risk'] = 'medium'
            else:
                btc_analysis['btc_volatility_risk'] = 'low'
            
            # XRP와의 상관관계 강도 분석
            correlation_strength = self._calculate_xrp_btc_correlation_v8(market_data, btc_data)
            btc_analysis['correlation_strength'] = correlation_strength
            
            # 최종 값 설정
            btc_analysis['btc_1h_change'] = btc_1h_change
            btc_analysis['btc_4h_change'] = btc_4h_change
            
            self.logger.log_with_context(
                'info',
                'v8.0 실제 BTC 데이터 분석 완료',
                btc_1h_change=f'{btc_1h_change:+.2f}%',
                btc_4h_change=f'{btc_4h_change:+.2f}%',
                influence=btc_analysis['btc_influence'],
                trend=btc_analysis['btc_trend'],
                volatility_risk=btc_analysis['btc_volatility_risk'],
                current_btc_price=f'{current_btc_price:,.0f}원'
            )
            
            return btc_analysis
            
        except Exception as e:
            self.logger.log_with_context('error', f'v8.0 실제 BTC 분석 중 오류: {e}')
            # 오류 시 기존 추정 방식으로 폴백
            return self._fallback_btc_analysis_v8(market_data)

    def _calculate_xrp_btc_correlation_v8(self, market_data: Dict, btc_data: pd.DataFrame) -> str:
        """
        XRP와 BTC의 상관관계 강도 계산
        
        Args:
            market_data (Dict): XRP 시장 데이터
            btc_data (pd.DataFrame): BTC 가격 데이터
            
        Returns:
            str: 상관관계 강도 ('high', 'medium', 'low')
        """
        try:
            # XRP 1시간봉 데이터 가져오기
            indicators = market_data.get('technical_indicators', {})
            xrp_1h = indicators.get('1h', {})
            
            if not xrp_1h or len(btc_data) < 10:
                return 'unknown'
            
            # 최근 10시간 변동률 비교
            btc_returns = btc_data['close'].pct_change().tail(10)
            
            # XRP 변동성 추정 (실제 데이터가 없으므로 추정)
            xrp_volatility = xrp_1h.get('volatility', {}).get('atr_ratio', 1.0)
            
            # 상관관계 추정
            if xrp_volatility > 2.0:  # 높은 변동성
                return 'high'
            elif xrp_volatility > 1.0:
                return 'medium'
            else:
                return 'low'
                
        except Exception as e:
            self.logger.log_with_context('error', f'XRP-BTC 상관관계 계산 중 오류: {e}')
            return 'unknown'

    def _fallback_btc_analysis_v8(self, market_data: Dict) -> Dict:
        """
        BTC 데이터 조회 실패 시 XRP 변동성 기반 추정
        ...
        try:
            ...
            self.logger.log_with_context(
                'warning',
                'v8.0 BTC 데이터 조회 실패 - XRP 변동성 기반 추정 사용',
                estimated_influence=btc_analysis['btc_influence'],
            ...
            self.logger.log_with_context('error', f'v8.0 BTC 폴백 분석 중 오류: {e}')
            return {
                'btc_influence': '알수없음', 
                'btc_1h_change': 0,
                'btc_4h_change': 0,
    ...
    def get_optimized_market_data(self) -> Optional[Dict]:
        """
        v8.0 API 최적화된 시장 데이터 수집 (BTC 실제 데이터 포함)
        리샘플링을 통해 API 호출 최소화 + BTC 정확도 확보
        ...
        try:
            ...
            # BTC 분석 호출
            # market_data 임시 구성해서 BTC 분석에 전달
            temp_market_data = {
                'current_price': current_price,
                'technical_indicators': technical_indicators
            }
            ...
            market_data = {
                ...
                'technical_indicators': technical_indicators,
                'xrp_expert_analysis': xrp_expert_analysis,
                'api_calls_used': 4,  # orderbook, balances, xrp_ohlcv, btc_ohlcv
                'data_collection_duration': time.time() - start_time
            }
            ...
            self.logger.log_with_context(
                'info',
                'v8.0 BTC 실제 데이터 포함 시장 데이터 수집 완료',
                api_calls=4,  # 1개 증가
                duration=f'{market_data["data_collection_duration"]:.2f}s',
                current_price=f'{current_price:,.0f}원',
                system_state=current_state.value
    ...
    def _request_ai_management_analysis_v8(self, market_data: Dict, trigger_analysis: Dict) -> Optional[Dict]:
        ...
        try:
                    ...
                    "json_schema": {
                        ...
                        "schema": {
                            ...
                            "properties": {
                                "new_target_price": {
                                    "type": "number",
                                    "description": "새로운 목표 가격 (원 단위)"
                                },
                                "new_stop_loss_price": {
                                    "type": "number",
                                    "description": "새로운 손절 가격 (원 단위)"
                                },
                                "target_change_reason": {
                                    "type": "string",
                                    "description": "목표가 변경 근거"
                                },
                                "stop_loss_change_reason": {
                                    "type": "string",
                                    "description": "손절가 변경 근거"
                                },
                                "management_strategy": {
                                    "type": "string",
                                    "description": "포지션 관리 전략"
                                },
                                "lessons_applied": {
                                    "type": "string",
                                    "description": "이번 조정에 적용한 교훈"
                                },
                                "risk_assessment": {
                                    "type": "object",
                                    "properties": {
                                        "expected_outcome": {
                                            "type": "string",
                                            "description": "조정 후 예상 결과"
                                        },
                                        "max_loss_percent": {
                                            "type": "number",
                                            "description": "최악의 경우 최대 손실률"
                                        },
                                        "max_profit_percent": {
                                            "type": "number",
                                            "description": "최선의 경우 최대 수익률"
                                        }
                                    },
                                    "required": ["expected_outcome", "max_loss_percent", "max_profit_percent"],
                                    "additionalProperties": False
                                }
                            },
                            "required": [
                                "new_target_price", "new_stop_loss_price",
                                "target_change_reason", "stop_loss_change_reason",
                                "management_strategy", "lessons_applied", "risk_assessment"
                            ],
                            "additionalProperties": False
                        }
                    }
            ...
            try:
                ai_response = response.choices[0].message.content
                parsed_management = json.loads(ai_response)
                
                # 디버그 로그
                self.logger.log_with_context(
                    'debug',
                    'AI 관리 JSON 응답 파싱 성공',
                    new_target=parsed_management.get('new_target_price'),
                    new_stop=parsed_management.get('new_stop_loss_price')
                )
                
            except json.JSONDecodeError as e:
                self.logger.log_with_context('error', f'AI 관리 JSON 응답 파싱 실패: {e}')
                self.logger.log_with_context('error', f'AI 원본 응답: {response.choices[0].message.content}')
                return None
            
            # 기존 _parse_ai_management_response_v8 대신 직접 처리
            current_price = market_data['current_price']
            
            # 관리 변경 유효성 검증
            if not self._validate_management_prices_v8(
                current_price,
                parsed_management['new_target_price'],
                parsed_management['new_stop_loss_price']
            ):
                return None
            ...
            final_management = {
                ...
                'target_price': parsed_management['new_target_price'],
                'stop_loss_price': parsed_management['new_stop_loss_price'],
                'target_reason': parsed_management['target_change_reason'],
                'stop_loss_reason': parsed_management['stop_loss_change_reason'],
                'sell_strategy': parsed_management['management_strategy'],
                'lessons_applied': parsed_management['lessons_applied'],
            ...
            self.logger.log_with_context(
                ...
                new_target=f'{final_management["target_price"]:,.0f}원',
                new_stop=f'{final_management["stop_loss_price"]:,.0f}원',
                trigger=trigger_analysis['trigger_type']
            )
            
            return final_management
...
목표: v8.0 시스템이 과거 경험을 통해 지속적으로 발전할 수 있는 학습 기반 구축"""
    ...
    def run_strategy_analysis_v8(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.log_with_context(
                    'info',
                    'orient_and_decide_v8 호출 전',
                    has_data=True,
                    state=self.state_manager.get_state()[0].value
                )
                ...
                if strategy:
                    ...
                    if trade_id:
                        ...
                        self.logger.log_with_context('info', 'v8.0 거래 계획 저장 불필요 또는 실패')
                else:
                    # 🔧 추가: 전략 없음 로그
                    self.logger.log_with_context(
                        'info',
                        'v8.0 전략 생성 결과 없음 - 현재 시장 상황에서 대기'
                    )
                
                # 성능 추적
                analysis_duration = time.time() - start_time
                api_calls_used = market_data.get('api_calls_used', 0) if market_data else 0
    ...
    def _check_and_update_analysis_interval_v8(self) -> None:
            ...
            try:
                ...
                self.logger.log_with_context('info', 'v8.0 동적 분석 주기 재평가 시작')
    ...
    def _monitor_price_with_state_management_v8(self) -> None:
        ...
        try:
            ...
            if is_spike and not self._is_emergency_cooldown_active_v8():
                ...
                self.logger.log_with_context('info', f'v8.0 {spike_type} 감지되었으나 쿨다운 중 - 무시')
